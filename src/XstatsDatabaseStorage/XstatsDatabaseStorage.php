...
namespace Drupal\xstats\XstatsDatabaseStorage;
 ...
 */
class XstatsDatabaseStorage implements StatisticsStorageInterface {
  ...
  */
  ...
  public function __construct(Connection $connection, StateInterface $state, RequestStack $request_stack) {
    if(\Drupal::config('xstats.settings')->get('xstats_active') == true) {
        \Drupal::messenger()->addMessage('Xstats Active');
    } else {
        \Drupal::messenger()->addMessage('Xstats Not active');
    }

    if(\Drupal::config('xstats.settings')->get('xstats_active') == false) {
        $this->connection = $connection;
    } else {
        // Use Xstats connection instead
        // $this->connection = new XstatsConnection;
    }
    $this->state = $state;
